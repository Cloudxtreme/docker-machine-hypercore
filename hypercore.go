...
func (d *Driver) GetCreateFlags() []mcnflag.Flag {
	return []mcnflag.Flag{
		mcnflag.Flag{
			EnvVar: "HYPERCORE_BOOT2DOCKER_URL",
			Name:   "hypercore-boot2docker-url",
			Usage:  "The URL of the boot2docker image. Defaults to the latest available version",
			Value:  "",
		},
		mcnflag.Flag{
			EnvVar: "HYPERCORE_CPU_COUNT",
			Name:   "hypercore-cpu-count",
			Usage:  "Number of CPUs for the machine (-1 to use the number of CPUs available)",
			Value:  1,
		},
		mcnflag.Flag{
			EnvVar: "HYPERCORE_MEMORY_SIZE",
			Name:   "hypercore-memory",
			Usage:  "Size of memory for host in MB",
			Value:  1024,
		},
		mcnflag.Flag{
			EnvVar: "HYPERCORE_DISK_SIZE",
			Name:   "hypercore-disk-size",
...
func (d *Driver) SetConfigFromFlags(flags drivers.DriverOptions) error {
	d.Boot2DockerURL = flags.String("hypercore-boot2docker-url")
	d.CPU = flags.Int("hypercore-cpu-count")
	d.Memory = flags.Int("hypercore-memory")
	d.DiskSize = flags.Int("hypercore-disk-size")
...
func (d *Driver) Create() error {
	if _, err := hypercoreExec("init"); err != nil {
		return err
	}
	...
	// }
	//
	// log.Debugf("Running hypercore boot2docker VM...")
...
func (d *Driver) Stop() error { // TODO
	// hypercore("controlvm", d.MachineName, "acpipowerbutton")
	for {
		s, err := d.GetState()
		if err != nil {
			return err
...
func (d *Driver) Remove() error { // TODO
	...
	//return hypercore("unregistervm", "--delete", d.MachineName)
	return nil
}
